...
async function processSingleActivation(activationId, isProduction) {
  ...
  try {
    ...
    // Fetch owner data for variable substitution
    let ownerData = null;
    const ownerId = (data.ownerId || data.userId || '').toLowerCase().trim();
    if (ownerId) {
        try {
            ...
            logger.warn(`[ProcessSequenceActivations] Failed to fetch owner ${ownerId}:`, e);
        }
    }

    const substituteEmailVariables = (text, contact, owner) => {
        if (!text) return text;
        let result = text;
        
        // Contact variables
        const c = contact || {};
        result = result.replace(/{{contact\.first_name}}/g, c.firstName || '')
                       .replace(/{{contact\.firstName}}/g, c.firstName || '')
                       .replace(/{{contact\.last_name}}/g, c.lastName || '')
                       .replace(/{{contact\.lastName}}/g, c.lastName || '')
                       .replace(/{{contact\.full_name}}/g, `${c.firstName || ''} ${c.lastName || ''}`.trim())
                       .replace(/{{contact\.name}}/g, `${c.firstName || ''} ${c.lastName || ''}`.trim())
                       .replace(/{{contact\.email}}/g, c.email || '')
                       .replace(/{{contact\.company}}/g, c.company || c.companyName || '')
                       .replace(/{{contact\.companyName}}/g, c.company || c.companyName || '')
                       .replace(/{{contact\.title}}/g, c.title || c.jobTitle || '')
                       .replace(/{{contact\.jobTitle}}/g, c.title || c.jobTitle || '')
                       .replace(/{{contact\.city}}/g, c.city || '')
                       .replace(/{{contact\.state}}/g, c.state || '');

        // Owner variables (if available)
        if (owner) {
             result = result.replace(/{{sender\.first_name}}/g, owner.firstName || owner.name?.split(' ')[0] || '')
                            .replace(/{{sender\.last_name}}/g, owner.lastName || owner.name?.split(' ').slice(1).join(' ') || '')
                            .replace(/{{sender\.full_name}}/g, owner.name || `${owner.firstName || ''} ${owner.lastName || ''}`.trim())
                            .replace(/{{sender\.email}}/g, owner.email || ownerId || '')
                            .replace(/{{sender\.title}}/g, owner.title || owner.jobTitle || '');
        }
    ...
    if (isEmailStep) {
      ...
      const emailCreates = validContacts.map(async (contact) => {
        ...
        const isManualMode = firstActiveStep.type === 'manual-email' || 
                           firstActiveStep.data?.mode === 'manual' || 
                           firstActiveStep.data?.isBuilderEmail === true;

        const payload = {
          id: emailId,
          type: 'scheduled',
          // If manual/builder email, set to pending_approval to skip AI generation
          status: isManualMode ? 'pending_approval' : 'not_generated',
          ...
          activationId,
          // Use provided content for manual emails, fallback to prompt for AI
          subject: isManualMode ? substituteEmailVariables(firstActiveStep.data?.subject || '', contact, ownerData) : undefined,
          html: isManualMode ? substituteEmailVariables(firstActiveStep.data?.body || '', contact, ownerData) : undefined,
          mode: isManualMode ? 'manual' : (defaultAiMode || 'standard'),
