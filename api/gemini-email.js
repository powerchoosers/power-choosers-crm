...
- "Cold email to a lead I could not reach by phone": This is a COLD email to someone you have NEVER spoken with. Structure: 1) Personal greeting + time awareness, 2) "I recently spoke with ${colleagueInfo?.found ? colleagueInfo.name : 'a colleague'} at ${company || 'your company'} and wanted to connect with you as well" + brief value proposition, 3) ONE call-to-action asking if they'd be open to a brief call - NEVER say "following up on our call" or reference any conversation with this specific person. CRITICAL: Do NOT repeat the company name multiple times in the same sentence. Use the company name only once per sentence`;
