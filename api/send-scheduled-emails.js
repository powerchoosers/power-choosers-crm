...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { immediate, emailId } = req.body || {};
    ...
    for (const emailDoc of readyToSendSnapshot.docs) {
      try {
        ...
        const isHtmlTemplate = finalHtml.includes('<!DOCTYPE html>') || (finalHtml.includes('<html') && finalHtml.includes('</html>'));
        const isHtmlBuilderEmail = emailData.isBuilderEmail === true;

        // Add signature if enabled in settings
        if (emailSettings.content.includeSignature) {
          if (isHtmlTemplate || isHtmlBuilderEmail) {
            // HTML template or HTML builder emails: Keep hardcoded signature (don't modify)
