...
- **Active Call Manager** - Header-based call tracker with real-time contact/account rendering and navigation
...
- **Apollo Enrichment (Cache-first)** - Company/contact enrichment with credit-safe cache and on-demand reveal
...
- **Dynamic Call Scripting (PEACE Framework)** - Intelligent, multi-stage script generation:
    - **P-O-S-P-C-S Structure**: Pre-call, Opener, Situation, Problem, Solution, Close, Settle.
    - **Discovery Hooks**: Replaces "unknown/unaware" statements with industry-specific diagnostic questions when data is missing.
    - **State-Specific Intelligence**: Automatically adjusts strategy for Texas (ERCOT mandatory shopping) vs. other deregulated states (Utility Default/Price to Compare).
    - **Interactive Problems**: Mandatory open-ended follow-up questions in the Problem section to encourage fluid client conversation.
    - **Visual Sectioning**: Color-coded, regex-parsed UI with key fact highlighting (dates, suppliers, rates).
    - **UI Standards**: Standardized AI "sparkles" icon (white), auto-scroll to script on generation, and streamlined sidebar (no clear/copy buttons).
...
3. **Configure environment variables**
   Create a `.env` file in the project root (see `local-env-setup.txt` for notes), then set:
   ```bash
...
4. **Start the development server**
   ...
   Port policy (local dev):
   - Always run the server on `http://localhost:3000`.
   - Do not start servers on other ports (e.g. 3001), since APIs and local URLs assume `:3000`.
   - If `3000` is busy, stop the other running server/process using it and restart on `3000`.
...
├── mcp-server/                 # Optional debugger MCP server
├── styles/
│   ├── main.css               # Application styles
│   ├── public.css             # Public/landing page styles
│   └── training-tool-animations.css # Training tool styles
├── scripts/
...
│   ├── background-loader-coordinator.js # Background loader orchestration
│   ├── pages/                 # Page-specific modules
│   │   ├── (dashboard in main.js)
...
│   │   ├── prospecting.js     # Prospecting page
...
│   │   ├── track/            # Email open tracking endpoints
│   │   │   └── [id].js      # Open tracking pixel endpoint
│   │   ├── click/            # Email click tracking endpoints
│   │   │   └── [id].js      # Click tracking redirect endpoint
│   │   ├── inbound-email.js # Inbound email processing (SendGrid webhook)
│   │   ├── backfill-threads.js # Backfill missing Gmail thread IDs
│   │   ├── stats.js           # Email analytics endpoints
...
│   │   ├── phone-retrieve.js
│   │   ├── phone-webhook.js
│   │   ├── search-organizations.js
│   │   ├── search-people.js
│   │   └── usage.js
│   ├── debug/                # Debug endpoints (optional)
│   │   ├── call.js
│   │   ├── health.js
│   │   └── log.js
│   ├── algolia/              # Algolia search endpoints
│   │   └── reindex.js         # Algolia reindex endpoint
│   ├── maps/                 # Google Maps integration
│   │   └── config.js          # Maps configuration endpoint
│   ├── posts/                # News/content endpoints
│   │   ├── generate-ai.js
│   │   ├── generate-static.js
│   │   └── list.js
│   ├── search.js             # Global search
│   ├── upload/               # File upload handling
│   │   ├── host-google-avatar.js
│   │   └── signature-image.js
...
- Widget provides company and contact enrichment with on-demand email/phone reveal
- Backend endpoints live under `api/apollo/` (company, contacts, enrich, search, phone retrieve/webhook, usage, health)
- Results are cached (localStorage + Firestore) to reduce API calls and credit usage
