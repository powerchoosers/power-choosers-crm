...
class PowerChoosersCRM {
    ...
    async getLinkedInTasksFromSequences() {
        ...
        try {
            let allFirebaseTasks = window.BackgroundTasksLoader && typeof window.BackgroundTasksLoader.getTasksData === 'function'
                ? (window.BackgroundTasksLoader.getTasksData() || [])
                : [];
            if (!Array.isArray(allFirebaseTasks) || allFirebaseTasks.length === 0) {
                try {
                    if (window.CacheManager && typeof window.CacheManager.getFromCache === 'function') {
                        const cached = await window.CacheManager.getFromCache('tasks');
                        allFirebaseTasks = Array.isArray(cached) ? cached : [];
                    }
                } catch (_) { }
            }
            if (!Array.isArray(allFirebaseTasks) || allFirebaseTasks.length === 0) {
        ...
        const getBackgroundTasks = async () => {
            try {
                if (window.BackgroundTasksLoader && typeof window.BackgroundTasksLoader.forceReload === 'function') {
                    await window.BackgroundTasksLoader.forceReload(false);
                }
                if (window.BackgroundTasksLoader && typeof window.BackgroundTasksLoader.getTasksData === 'function') {
                    ...
                    return Array.isArray(bgTasks) ? bgTasks : [];
                }
            } catch (_) { }
            return [];
        };
        ...
        if (!skipFirebase) {
            try {
...

                {
                    ...
                    if (processedTasksFiltered.length > 0 && (!window.BackgroundTasksLoader || typeof window.BackgroundTasksLoader.getTasksData !== 'function')) {
