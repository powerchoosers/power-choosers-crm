...
class CacheManager {
      ...
      return new Promise((resolve) => {
        request.onsuccess = () => {
          ...
          if (collection === 'tasks') {
            ...
            expiry = this.sequencesCacheExpiry;
          } else {
  ...
  async fetchFromFirestore(collection) {
    ...
    try {
      ...
      if (collection === 'accounts' && window.DataManager && typeof window.DataManager.queryWithOwnership === 'function') {
        try {
          const data = await window.DataManager.queryWithOwnership('accounts');
          logFetch(Array.isArray(data) ? data.length : 0);
          return data;
        } catch (error) {
