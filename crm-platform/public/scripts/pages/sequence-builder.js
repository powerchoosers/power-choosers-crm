  ...
  async sendEmail(email) {
    try {
      ...
      const deliverability = {
        ...
        // Force disable signature if it's a builder email
        signatureImageEnabled: step?.data?.isBuilderEmail ? false : (emailSettings.content?.signatureImage !== false)
      };
...
(function () {
  ...
  async function searchContacts(query) {
    ...
    try {
      ...
      const existingContactIds = state.contacts.map(c => normalizeId(c.id));
      ...
      if (filteredContacts.length === 0) {
        ...
        const resultsHtml = filteredContacts.map(contact => {
          ...
          const isAlreadyAdded = existingContactIds.includes(normalizeId(person.id));
          return `
  ...
  async function addContactToSequence(contact) {
    ...
    if (state.contacts.some(c => normalizeId(c.id) === normalizeId(contact.id))) {
    ...
    state.membersVersion++;
    state.stepsTotalCount = state.contacts.length;
    if (state.currentSequence) {
      state.currentSequence.recordCount = state.contacts.length;
      if (state.currentSequence.stats) {
        state.currentSequence.stats.active = state.contacts.length;
      }
    }
    ...
    try {
      ...
      // Update sequence document: increment counts and update contacts array
      const updateData = {
        contacts: state.contacts,
        updatedAt: Date.now(),
        // Use increment for atomic updates to ensure accuracy
        "stats.active": window.firebase.firestore.FieldValue.increment(1),
        "recordCount": window.firebase.firestore.FieldValue.increment(1)
      };
    ...
    refreshSequenceContactsPanel();
    refreshContactSearchResultsIfOpen();
    render();

      // Dispatch sequence member added event
      try {
        document.dispatchEvent(new CustomEvent('pc:sequence-member-added', {
          detail: { 
            sequenceId: state.currentSequence.id, 
      ...
      try {
        ...
        if (hasActiveMembers || hasExistingActivations) {
          if (contact.email && !contact._skipEmailSteps) {
            ...
            // Get user email for ownership (reuse same function from above)
            const getUserEmailForActivation = () => {
              try {
                if (window.DataManager && typeof window.DataManager.getCurrentUserEmail === 'function') {
                  const email = window.DataManager.getCurrentUserEmail();
                  if (email && typeof email === 'string' && email.trim()) {
            ...
            const activationUserEmail = getUserEmailForActivation();
            ...
            const sequenceActivationData = {
              ...
              totalContacts: 1,
              // CRITICAL: Set ownership fields for Firestore rules compliance
              ownerId: activationUserEmail,
              assignedTo: activationUserEmail,
              createdBy: activationUserEmail,
  ...
  function updateContactCountDisplay() {
    ...
    // Update header badge
    const contactCountEls = document.querySelectorAll('.contact-count-display');

    contactCountEls.forEach(el => {
  ...
  function invalidateSequenceMembersCache(sequenceId) {
    if (window._sequenceMembersCache && sequenceId) {
      const cacheKey = `sequence-members-${sequenceId}`;
      window._sequenceMembersCache.delete(cacheKey);
    }
  }

  function refreshContactSearchResultsIfOpen() {
  ...
  function refreshSequenceContactsPanel() {
    ...
    if (titleEl) {
      titleEl.textContent = `Sequence Contacts (${contacts.length})`;
    }
  }

  async function handleSequenceMemberEvent(action, detail) {
    if (!detail || detail.source === 'sequence-builder') return;
    const currentSequenceId = normalizeId(state.currentSequence?.id);
    const eventSequenceId = normalizeId(detail.sequenceId);
    if (!currentSequenceId || currentSequenceId !== eventSequenceId) return;

    const contactId = normalizeId(detail.contactId);
    let needsReload = false;

    if (action === 'removed') {
      if (contactId) {
        const idx = state.contacts.findIndex(c => normalizeId(c.id) === contactId);
        if (idx >= 0) state.contacts.splice(idx, 1);

        if (state.stepsLoadedMembers && state.stepsLoadedMembers.length > 0) {
          const stepIdx = state.stepsLoadedMembers.findIndex(m => normalizeId(m.targetId) === contactId);
          if (stepIdx >= 0) state.stepsLoadedMembers.splice(stepIdx, 1);
        }
      } else {
        needsReload = true;
      }
    ...
    if (action === 'added') {
      if (contactId) {
        const exists = state.contacts.some(c => normalizeId(c.id) === contactId);
        if (!exists && detail.contact) {
          state.contacts.push(detail.contact);
        } else if (!exists) {
          needsReload = true;
        }
      } else {
    ...
    invalidateSequenceMembersCache(eventSequenceId);

    if (needsReload) {
      await loadContactsFromSequenceMembers(eventSequenceId);
    }
    ...
    if (state.currentSequence) {
      ...
      if (state.currentSequence.stats) {
        if (needsReload) {
          ...
          const currentActive = state.currentSequence.stats.active || 0;
          if (action === 'removed') {
            state.currentSequence.stats.active = Math.max(0, currentActive - 1);
          } else if (action === 'added') {
            state.currentSequence.stats.active = currentActive + 1;
          }
        }
      }
    }
    ...
    if (state.stepsLoadedMembers && state.stepsLoadedMembers.length > 0) {
      ...
      if (state.sequenceMembersCache && state.sequenceMembersCache.sequenceId === eventSequenceId) {
        state.sequenceMembersCache.data = state.stepsLoadedMembers.slice();
        state.sequenceMembersCache.timestamp = Date.now();
      }
    }
    ...
    refreshSequenceContactsPanel();
    refreshContactSearchResultsIfOpen();
    const panelOpen = !!document.getElementById('sequence-contacts-panel');
  ...
  function show(sequence, options = {}) {
    ...
    if (seqRecordCount === 0 && state.contacts.length > 0 && !shouldPreserveState) {
      state.contacts = [];
    }
  ...
  async function loadContactsFromSequenceMembers(sequenceId, options = {}) {
    try {
        ...
        if (attempt < 6) {
          setTimeout(() => {
            if (state.currentSequence?.id === sequenceId) {
              loadContactsFromSequenceMembers(sequenceId, { attempt: attempt + 1 });
            }
          }, 250);
        }
        return;
      ...
      <div class="page-header" id="sequence-builder-header">
        ...
        <div class="page-actions">
          ...
          <button class="quick-action-btn list-header-btn" id="contacts-btn" aria-label="Sequence Contacts" title="Sequence Contacts" aria-haspopup="dialog">
            ...
            <span class="contact-count-display badge-count" style="display: none; margin-left: 6px; font-size: 12px; font-weight: 500;"></span>
          </button>
      ...
      const removeOne = async (id) => {
        ...
        try {
          ...
          if (db && state.currentSequence?.id) {
            ...
            if (id && state.currentSequence?.id) {
              try {
                // 1. Delete tasks
                const tasksSnapshot = await db.collection('tasks')
                  .where('contactId', '==', id)
                  ...
                  taskIds.forEach(taskId => {
                    ...
                    try {
                      document.dispatchEvent(new CustomEvent('pc:task-deleted', {
                        detail: { taskId, source: 'sequence-builder' }
                      }));
                    } catch (_) { }
                  });
                }

                // 2. Delete sequenceActivations (prevents re-add issues)
                const activationsSnapshot = await db.collection('sequenceActivations')
                  .where('sequenceId', '==', state.currentSequence.id)
                ...
                if (!activationsSnapshot.empty) {
                  ...
                  console.log('[SequenceBuilder] Deleted old sequenceActivations for contact:', id);
                }

                // 3. Delete scheduledEmails (prevents "pending_approval" stuck state)
                const emailsSnapshot = await db.collection('scheduledEmails')
                  .where('sequenceId', '==', state.currentSequence.id)
                  .where('contactId', '==', id)
                  .get();
                ...
                if (!emailsSnapshot.empty) {
                  ...
                  console.log('[SequenceBuilder] Deleted old scheduledEmails for contact:', id);
                }

              } catch (cleanupError) {
                console.warn(`[SequenceBuilder] Failed to cleanup sequence data for contact ${id}:`, cleanupError);
        ...
        // Update state
        const idx = state.contacts.findIndex(c => String(c.id) === normalizedId);
        if (idx >= 0) {
          state.contacts.splice(idx, 1);
        }

        // Update state.stepsLoadedMembers if it exists (for pagination consistency)
        if (state.stepsLoadedMembers) {
           const stepIdx = state.stepsLoadedMembers.findIndex(m => String(m.targetId) === normalizedId);
           if (stepIdx >= 0) {
              state.stepsLoadedMembers.splice(stepIdx, 1);
           }
        }

        // Update sequenceMembersCache to prevent stale data reappearing
        if (state.sequenceMembersCache && state.sequenceMembersCache.sequenceId === state.currentSequence.id) {
             state.sequenceMembersCache.data = (state.stepsLoadedMembers || []).slice();
        }

        // Update local recordCount
        if (state.currentSequence) {
          state.currentSequence.recordCount = state.contacts.length;
          // Update stats.active locally
          if (state.currentSequence.stats) {
             const currentActive = state.currentSequence.stats.active || 0;
             state.currentSequence.stats.active = Math.max(0, currentActive - 1);
          }
        ...
        // Dispatch sequence member removed event
        try {
          document.dispatchEvent(new CustomEvent('pc:sequence-member-removed', {
            detail: { 
              sequenceId: state.currentSequence.id, 
              contactId: id,
              source: 'sequence-builder'
        ...
        // Update DOM
        // Try selector first
        let row = list ? list.querySelector(`.contact-row[data-id="${CSS.escape(normalizedId)}"]`) : null;
        
        // Fallback: direct attribute check if selector failed
        if (!row && list) {
          row = Array.from(list.querySelectorAll('.contact-row')).find(el => el.getAttribute('data-id') === normalizedId);
        }
        ...
        if (row) {
          ...
          if (list.children.length === 0) {
             list.innerHTML = `<div class="empty-text">No contacts in this sequence yet.</div>`;
          }
        } else {
        }
        ...
        // FORCE REFRESH: Reload members from server to ensure pagination and lists are in sync
        try {
           if (state.currentSequence?.id && typeof loadSequenceMembersPage === 'function') {
              console.log('[SequenceBuilder] Force reloading members page after removal...');
              await loadSequenceMembersPage(state.currentSequence.id, 1, true);
              // Re-render to update the main UI (if visible)
              render();
           }
        } catch (refreshErr) {
           console.warn('[SequenceBuilder] Failed to refresh members list:', refreshErr);
        }
      };
  ...
  function renderPreviewIframe(html) {
    if (!html) return '<div style="padding:20px; text-align:center; color:#666;">(empty)</div>';
    
    // Basic escaping for srcdoc attribute
    // Also strip min-height:100vh from body/bg to prevent excessive empty space in preview
    let srcdoc = String(html)
    ...
    return `
      <iframe 
        ...
        srcdoc="${srcdoc}" 
        style="width:100%; height:100%; min-height:400px; border:none; background:white; border-radius:4px;" 
          ...
          <div class="step-body" ${expanded ? '' : 'hidden'}>
            ...
            <div class="tab-panels">
              <div class="tab-panel ${editorActive ? '' : 'hidden'}" data-tab="editor">
                ...
                <div class="form-row">
                  ${htmlMode
          ? `<div class="html-editor-wrap"><div class="html-mode-badge" aria-hidden="true">HTML Mode</div><textarea class="textarea-dark body-input" data-editor="html" spellcheck="false" style="min-height: 350px; max-height: 600px; padding-bottom: 32px; overflow-y: auto !important; resize: vertical; background: #0f172a !important; color: #e2e8f0 !important;">${escapeHtml(step.data?.body || '')}</textarea></div>`
          : `<div class="rich-editor-host input-dark" style="padding:0; min-height:350px; display:flex; flex-direction:column; border: 1px solid var(--border-color); border-radius: 8px;">
              <template shadowrootmode="open">
                <style>
                  :host { display: block; flex: 1; display: flex; flex-direction: column; }
                  .body-input { 
                     flex: 1;
                     width: 100%; 
                     min-height: 350px; 
                     max-height: 800px;
                     padding-bottom: 32px; /* Ensure last line is visible */
                  ...
                  /* Email content resets to prevent bleeding and ensure standard look */
                  p { margin: 0.5em 0; }
                ...
                <div class="textarea-dark body-input" contenteditable="true" role="textbox" aria-multiline="true" data-editor="rich" placeholder="Email body...">${step.data?.body || ''}</div>
              </template>
             </div>`}
                </div>`}
              ...
              <div class="tab-panel ${previewActive ? '' : 'hidden'}" data-tab="preview">
                ...
                <div class="ai-compose">
                  ...
                  <div class="email-preview-container" style="display:flex; flex-direction:column; height:calc(100vh - 450px); min-height:600px;">
                    <div class="preview-header" style="flex:0 0 auto; margin-bottom:12px; padding:12px; background:var(--bg-card); border:1px solid var(--border-color); border-radius:8px;">
                       <div class="preview-label" style="font-size:11px; text-transform:uppercase; letter-spacing:0.05em; color:var(--text-secondary); margin-bottom:4px;">Subject</div>
                       <div class="preview-subject" style="font-size:14px; font-weight:500; color:var(--text-primary);">${escapeHtml((step.data?.aiOutput?.subject) || (previewSubject || '(no subject)'))}</div>
                    </div>
                    <div class="preview-viewport" style="flex:1; overflow:hidden; border:1px solid var(--border-color); border-radius:8px; background:#fff;">
                      ${renderPreviewIframe(((step.data?.aiOutput?.html) || previewBodyHtml || '') + getSignatureForPreview(step, isAuto))}
                    </div>
                  </div>
            ...
            : escapeHtml((step.data?.aiStatus || 'draft').toUpperCase())}
                ...
                <div class="email-preview-container" style="display:flex; flex-direction:column; height:calc(100vh - 450px); min-height:600px;">
                  <div class="preview-header" style="flex:0 0 auto; margin-bottom:12px; padding:12px; background:var(--bg-card); border:1px solid var(--border-color); border-radius:8px;">
                     <div class="preview-label" style="font-size:11px; text-transform:uppercase; letter-spacing:0.05em; color:var(--text-secondary); margin-bottom:4px;">Subject</div>
                     <div class="preview-subject" style="font-size:14px; font-weight:500; color:var(--text-primary);">${escapeHtml(previewSubject || '(no subject)')}</div>
                  </div>
                  <div class="preview-viewport" style="flex:1; overflow:hidden; border:1px solid var(--border-color); border-radius:8px; background:#fff;">
                    ${renderPreviewIframe((previewBodyHtml || '') + getSignatureForPreview(step, isAuto))}
                  </div>
                </div>`}
              </div>
            </div>
...
2. TONE OPENER (CREATIVE FREEDOM WITH VARIETY):
   ...
   - **CREATIVE FREEDOM**: You have full creative freedom to craft a natural, conversational opener. The tone opener provided by the system is INSPIRATION only - use it as a stylistic guide, not a template.
   - **VARIETY IS CRITICAL**: Vary your opener style across emails. Mix between direct questions, soft curiosity, peer observations, and honest/direct styles.
   - If system provides tone opener, use it as stylistic inspiration (don't force-match it)
   ...
   - Vary the opener - do NOT repeat the same pattern every time
...
PURPOSE: Clear final touchpoint - give them an out or a last chance to engage`;
  ...
  function getSignatureForPreview(step, isAuto) {
    try {
      // Skip signature for builder-generated emails
      if (step?.data?.isBuilderEmail) {
        return '';
      }
...
<div contenteditable="false" data-signature="true" style="margin-top: 28px; padding-top: 24px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  function selectionIsInside(el) {
    ...
    const sel = (root instanceof ShadowRoot) ? root.getSelection() : (window.getSelection ? window.getSelection() : null);
  ...
  function insertTokenAsChip(editor, scope, key) {
    ...
    // Need to trigger save - persistRich takes step, but here we might not have it easily?
    // Actually insertTokenAsChip is called from context where we can trigger save or use 'input' event.
    // The caller usually handles step data update or we rely on 'input' event bubbling?
    // 'DOMSubtreeModified' is deprecated.
    // Let's rely on persistRich if it works, or trigger an input event.
  ...
  function updateToggleStates(card, editor) {
    ...
    card.querySelectorAll('.formatting-bar .fmt-btn').forEach(b => {
      ...
      if (fmt && (fmt in map)) {
        let on = false;
        const root = editor.getRootNode();
        // NOTE: queryCommandState operates on document, might fail in Shadow DOM.
        // However, Shadow DOM specs say execCommand/queryCommandState on document *should* work if focus is inside.
        // We will try document.queryCommandState first.
  ...
  function attachEvents() {
    ...
    if (!document._sequenceBuilderMemberEventsBound) {
      document._sequenceBuilderMemberEventsBound = true;
      const onMemberRemoved = (e) => handleSequenceMemberEvent('removed', e.detail);
      const onMemberAdded = (e) => handleSequenceMemberEvent('added', e.detail);
      document.addEventListener('pc:sequence-member-removed', onMemberRemoved);
      document.addEventListener('pc:sequence-member-added', onMemberAdded);
    }
    ...
    // Helper to find editor (handling Shadow DOM for rich text)
    function getEditorFromCard(card) {
      if (!card) return null;
      // Try finding HTML editor first (direct child)
      const htmlEditor = card.querySelector('.body-input[data-editor="html"]');
      if (htmlEditor) return htmlEditor;
      
      // Try finding Rich editor (in shadow root)
      const host = card.querySelector('.rich-editor-host');
      if (host && host.shadowRoot) {
        return host.shadowRoot.querySelector('.body-input');
      }
      
      // Fallback (old way)
      return card.querySelector('.body-input');
    }
  ...
  function attachBuilderEvents() {
        ...
        if (isCollapsed) {
          ...
          requestAnimationFrame(() => {
            ...
            setTimeout(() => {
              ...
              stepBody.style.overflow = 'visible';
            }, 400);
          });
    ...
    // Helper to attach editor listeners (works for Light or Shadow DOM editors)
    const attachEditorListeners = (inp) => {
      if (!inp) return;
      const persist = () => {
        // Need to find the step card.
        // If inp is in shadow DOM, we need to go up to host, then closest card
        let card = inp.closest('.step-card');
        if (!card) {
          const host = inp.getRootNode()?.host;
          if (host) card = host.closest('.step-card');
        }
    ...
    // 1. Attach to standard inputs (HTML mode or legacy)
    container.querySelectorAll('.step-card .body-input').forEach(attachEditorListeners);
    
    // 2. Attach to Shadow DOM inputs (Rich Text mode)
    container.querySelectorAll('.step-card .rich-editor-host').forEach(host => {
      // Manual hydration fallback for browsers/environments where declarative shadow DOM via innerHTML fails
      if (!host.shadowRoot) {
        ...
        if (tmpl) {
          try {
            ...
            console.error('[SequenceBuilder] Failed to hydrate Shadow DOM:', e);
          }
        }
      }

      if (host.shadowRoot) {
        const inp = host.shadowRoot.querySelector('.body-input');
        if (inp) attachEditorListeners(inp);
      }
    });
    ...
    container.querySelectorAll('.step-card .editor-toolbar .toolbar-btn').forEach(btn => {
      ...
      btn.addEventListener('mousedown', (e) => {
        const card = btn.closest('.step-card');
        const editor = getEditorFromCard(card);
      ...
      btn.addEventListener('click', (e) => {
        ...
        switch (action) {
          ...
          case 'templates': {
            try {
              if (window.PCEmailBuilder && typeof window.PCEmailBuilder.open === 'function') {
                const subjectInput = card.querySelector('.subject-input');
                const currentSubject = subjectInput ? (subjectInput.value || '') : (step.data?.subject || '');
                let initialHtml = '';
                if (isRich && editor) {
                  initialHtml = editor.innerHTML || '';
                } else if (isHtml && editor) {
                  initialHtml = editor.value || '';
                } else if (editor) {
                  initialHtml = editor.value || editor.innerHTML || '';
                }

                window.PCEmailBuilder.open({
                source: 'sequence-builder',
                subject: currentSubject,
                initialHtml: initialHtml,
                onSave: function (result) {
                    if (!step.data) step.data = {};
                    step.data.subject = (result && result.subject) || currentSubject;
                    step.data.body = (result && result.html) || initialHtml;
                    step.data.editorMode = 'html';
                    step.data.isBuilderEmail = true; // Flag to prevent auto-signature
                    step.activeTab = 'preview'; // Switch to preview to show the rendered result
                    render();
                    try { scheduleStepSave(step.id); } catch (_) { }
                  }
                });
              } else {
                window.crm && window.crm.showToast && window.crm.showToast('HTML email builder not available yet');
              }
            } catch (e) {
              console.error('[SequenceBuilder] Templates handler failed', e);
              try { window.crm && window.crm.showToast && window.crm.showToast('Unable to open templates'); } catch (_) { }
            }
            break;
          }
          case 'variables': {
    ...
    container.querySelectorAll('.step-card .ai-bar').forEach(bar => {
      ...
      // Add input/blur listeners to the AI prompt textarea to ensure it's saved
      const promptTextarea = bar.querySelector('.ai-prompt');
      if (promptTextarea) {
        const persistPrompt = () => {
          const id = card?.getAttribute('data-id');
          const step = state.currentSequence.steps.find(s => s.id === id);
          ...
          const newVal = promptTextarea.value.trim();
          if (step.data.aiPrompt !== newVal) {
            step.data.aiPrompt = newVal;
            try { 
        ...
        promptTextarea.addEventListener('input', () => {
          // AI prompt input changed
        });
        
        promptTextarea.addEventListener('blur', persistPrompt);
      }
        ...
        btn.addEventListener('click', async (e) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              const previewBodyEl = card.querySelector('.preview-viewport');
              ...
              if (previewBodyEl && html) {
                ...
                let iframe = previewBodyEl.querySelector('iframe');
                if (iframe) {
                  try {
                    ...
                    previewBodyEl.innerHTML = renderPreviewIframe(fullHtml);
                  }
                } else {
                  previewBodyEl.innerHTML = renderPreviewIframe(fullHtml);
                }
    ...
    if (!container.dataset.aiSaveDelegated) {
      container.addEventListener('click', (e) => {
        ...
        if (step.data.aiOutput) {
          ...
          if (previewBodyEl) {
            // Get HTML content, check for iframe first
            let bodyHtml = '';
            const iframe = previewBodyEl.querySelector('iframe');
            if (iframe) {
              try {
                const doc = iframe.contentDocument || iframe.contentWindow.document;
               ...
               bodyHtml = previewBodyEl.innerHTML || '';
            }
    ...
    container.querySelectorAll('.step-card').forEach(card => {
      ...
      const updatePreview = (contact) => {
        ...
        const updateIframe = (el, html) => {
             let iframe = el.querySelector('iframe');
             if (iframe) {
               try {
                 const doc = iframe.contentDocument || iframe.contentWindow.document;
                 doc.open(); doc.write(html); doc.close();
               } catch(e) { el.innerHTML = renderPreviewIframe(html); }
             } else {
               el.innerHTML = renderPreviewIframe(html);
             }
        };

        if (contact) {
          subjEl && (subjEl.innerHTML = substituteContactTokensInText(subjRaw, contact) || '(no subject)');
          bodyEl && updateIframe(bodyEl, (processBodyHtmlWithContact(bodyRaw, contact) || '') + signature);
        } else {
          subjEl && (subjEl.textContent = subjRaw || '(no subject)');
          bodyEl && updateIframe(bodyEl, (bodyRaw || '') + signature);
        }
      };
