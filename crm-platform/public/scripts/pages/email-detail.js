...
(function() {
  ...
  function actuallyPopulateEmailDetails(email) {
    ...
    if (els.emailContent) {
      ...
      const applyClosingBreaks = (html) => {
        ...
        out = out.replace(/(Best regards,)(?:\s|&nbsp;|&#160;|\u00A0)+([A-Z][a-z]+)/g, '$1<br>$2');
        out = out.replace(/(Regards,)(?:\s|&nbsp;|&#160;|\u00A0)+([A-Z][a-z]+)/g, '$1<br>$2');
        out = out.replace(/(Thanks,)(?:\s|&nbsp;|&#160;|\u00A0)+([A-Z][a-z]+)/g, '$1<br>$2');
        out = out.replace(/(Thank you,)(?:\s|&nbsp;|&#160;|\u00A0)+([A-Z][a-z]+)/g, '$1<br>$2');
        out = out.replace(/(Cheers,)(?:\s|&nbsp;|&#160;|\u00A0)+([A-Z][a-z]+)/g, '$1<br>$2');
      ...
      */
      ...
      const isHtmlBuilderEmail = /data-html-builder\s*=\s*(?:"true"|'true'|true)/i.test(htmlSource || rawHtml || '');
      const isFullHtmlDocument = /<!doctype\s+html|<html[\s>]/i.test(htmlSource || rawHtml || '');

      const renderHtmlInIframe = (html) => {
        if (!els.emailContent) return;
        const decoded = decodeQuotedPrintable(html || '');

        let srcdoc = decoded;
        if (!/<base\b/i.test(srcdoc)) {
          if (/<head[\s>]/i.test(srcdoc)) {
            srcdoc = srcdoc.replace(/<head(\s[^>]*)?>/i, (m) => `${m}<base target="_blank">`);
          } else {
            srcdoc = `<base target="_blank">${srcdoc}`;
          }
        }

        els.emailContent.innerHTML = '';
        ...
        els.emailContent.appendChild(iframe);
        ...
        const resize = () => {
          try {
            ...
            const nextHeight = Math.max(body.scrollHeight || 0, htmlEl.scrollHeight || 0, 520);
      ...
      if (isOurEmail && (isHtmlBuilderEmail || isFullHtmlDocument) && htmlSource) {
        renderHtmlInIframe(htmlSource);
        return;
      }
      ...
      if (shouldShowSignature) {
        try {
          ...
          if (signature) {
            ...
            const wrappedSignatureHtml = `
              <div class="crm-signature-wrapper" style="margin-top: 16px; border: none; padding: 0;">
                ${signatureHtml}
              </div>`;

            if (contentHtml.includes('class="crm-email-content"')) {
              const closeIdx = contentHtml.lastIndexOf('</div>');
              if (closeIdx !== -1) {
                contentHtml = contentHtml.slice(0, closeIdx) + wrappedSignatureHtml + contentHtml.slice(closeIdx);
              } else {
                contentHtml += wrappedSignatureHtml;
              }
            } else {
              contentHtml += wrappedSignatureHtml;
            }
      ...
      if (isOurEmail && email.type === 'scheduled' && contentHtml) {
        try {
          contentHtml = contentHtml.replace(/(Best regards,)(?!\s*<br>)[\s\u00A0]*([A-Z][a-z]+)/gi, '$1<br>$2');
          contentHtml = contentHtml.replace(/(Regards,)(?!\s*<br>)[\s\u00A0]*([A-Z][a-z]+)/gi, '$1<br>$2');
          contentHtml = contentHtml.replace(/(Thanks,)(?!\s*<br>)[\s\u00A0]*([A-Z][a-z]+)/gi, '$1<br>$2');
          contentHtml = contentHtml.replace(/(Thank you,)(?!\s*<br>)[\s\u00A0]*([A-Z][a-z]+)/gi, '$1<br>$2');
          contentHtml = contentHtml.replace(/(Cheers,)(?!\s*<br>)[\s\u00A0]*([A-Z][a-z]+)/gi, '$1<br>$2');
        } catch (_) {}
      }
      els.emailContent.innerHTML = contentHtml;

      // Use sent-email-preview styling for both sent and scheduled emails.
      // This keeps the scheduled approval/regenerate view visually consistent with the real sent email.
      const shouldUseSentPreview = isSentEmail || email.type === 'scheduled' || email.status === 'pending_approval' || email.status === 'approved';
      if (shouldUseSentPreview) {
  ...
  function cleanupEmailDom(container) {
    ...
    // Check if this is an HTML builder email (manual mode)
    // If so, we skip aggressive style stripping to preserve the custom design
    const isBuilderEmail = container.querySelector('[data-html-builder="true"]') || 
                          (container.innerHTML && container.innerHTML.includes('data-html-builder="true"'));
    
    if (isBuilderEmail) {
      // Still remove broken images, but preserve structure and styles
...
<body>
  ...
  async function approveScheduledEmail(emailId) {
    try {
      ...
      // Show success message
      if (window.crm && window.crm.showToast) {
        window.crm.showToast('Email approved. Click Send Now to send immediately.');
      }

      try {
        if (state.currentEmail && state.currentEmail.id === emailId) {
          state.currentEmail.status = 'approved';
          state.currentEmail.approvedAt = state.currentEmail.approvedAt || Date.now();
          state.currentEmail.updatedAt = new Date().toISOString();
        }
        addScheduledEmailActions();
      } catch (_) {
      }
    ...
    try {
      ...
      const wrappedHtml = `
        <div class="email-content-wrapper" style="
          ...
          font-size: 15px;
        ">
          <div data-body="true">${decodedHtml}</div>
        </div>
        <style>
  ...
  function renderOurHtmlEmail(html) {
    if (!html) return '<p style="margin: 0 0 16px 0; color: var(--text-primary, #ffffff);">No content available</p>';
    try {
      let decoded = decodeQuotedPrintable(html);
      let signatureBlock = '';

      // Remove scripts/iframes/event handlers/js urls but keep style/head
      // NOTE: We no longer remove tracking pixels here because they are often the only content in a <p> 
      // and removing them causes the paragraph to collapse in the preview.
      decoded = decoded
        .replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '')
        .replace(/<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi, '')
        .replace(/on\w+="[^"]*"/gi, '')
      ...
      if (isContentEditableHtml) {
        ...
        // Process the body: convert divs to paragraphs with proper spacing (16px bottom margin)
        decoded = decoded
          .replace(/<div><br\s*\/?><\/div>/gi, '<p style="margin: 0 0 16px 0; color: var(--text-primary, #ffffff);">&nbsp;</p>')
          .replace(/<div>([^<]*)<\/div>/gi, '<p style="margin: 0 0 16px 0; color: var(--text-primary, #ffffff);">$1</p>')
          .replace(/<br\s*\/?>/gi, '<br>');
        ...
        if (signatureHtml) {
          signatureBlock = `
            <div class="crm-signature-wrapper" style="margin-top: 16px; border: none; padding: 0;">
              <style>
                .crm-signature-wrapper table { border-collapse: collapse !important; border-spacing: 0 !important; margin: 0 !important; }
              </style>
              ${signatureHtml}
            </div>`;
        }
        ...
        // 1. Extract signature FIRST to protect it from body formatting rules
        let signatureHtml = '';
        const sigMatch = decoded.match(/(<div[^>]*data-signature="true"[^>]*>[\s\S]*)/i);
        if (sigMatch) {
          signatureHtml = sigMatch[1];
          decoded = decoded.substring(0, sigMatch.index);
        }

        // 2. Fix signature spacing in BODY (Best regards, etc.) - Force double break for clear separation
        // Enhanced regex to handle &nbsp; and HTML tags between greeting and name

        // 0. Convert literal newlines to <br> FIRST to normalize input
        // This handles cases where \n wasn't converted yet (e.g. Scheduled emails)
        decoded = decoded.replace(/([^>\s])(\r\n|\r|\n)([^<\s])/g, '$1<br>$3');
        decoded = decoded.replace(/>(\r\n|\r|\n)([^<\s])/g, '><br>$2');
        decoded = decoded.replace(/,(\r\n|\r|\n)([A-Z][a-z]+)/g, ',<br>$2');

        // 1. Explicitly handle the <br> pattern seen in logs: "Best regards,<br>Lewis"
        // Also handles "Best regards,Lewis" (no space) by matching 0+ spaces/brs
        decoded = decoded.replace(
          /(Best regards|Cheers|Thanks|Sincerely|Regards|Warm regards|Thank you)(,?)([\s\r\n\u00A0]*)(?:<br\s*\/?>)*([\s\r\n\u00A0]*)([A-Za-z][a-z]+)/gi,
          (match, greeting, comma, s1, s2, name) => {
            if (match.includes('</p><p')) return match;
            return `${greeting}${comma}<br>${name}`;
          }
        );
        
        // 3. Ensure <p> tags have spacing (BODY ONLY)
        decoded = decoded.replace(/<p([^>]*)>/gi, (match, attrs) => {
          if (!/style\s*=\s*["'][^"']*margin/i.test(attrs)) {
            if (/style\s*=\s*["']/i.test(attrs)) {
              return match.replace(/style\s*=\s*(["'])/i, 'style=$1margin: 0 0 16px 0; ');
            } else {
              return `<p style="margin: 0 0 16px 0; color: var(--text-primary, #ffffff);"${attrs}>`;
            }
          }
          return match;
        });

        // 4. Fix greeting run-on inside HTML text
        decoded = decoded.replace(/((?:Hi|Hello|Hey|Dear)\s+[^,<]{1,80},)\s*(?=[A-Z])/gi, '$1<br><br>');

        // 5. Re-attach signature with layout protection
        if (signatureHtml) {
             // If body has no block tags, wrap it
            const hasBlocks = /<\s*(p|br|div|table|tr|td|ul|ol|li|blockquote|hr)/i.test(decoded);
            if (!hasBlocks && decoded.trim().length > 0) {
                 decoded = `<p style="margin: 0 0 12px 0; color: var(--text-primary, #ffffff);">${escapeHtml(decoded)}</p>`;
            }
            
            // Wrap signature in a protective div that resets table styles
            // "wide spaced html signature" usually means inherited border-spacing or padding
            const wrappedSig = `
              <div class="crm-signature-wrapper" style="margin-top: 16px; border: none; padding: 0;">
                 <style>
                   .crm-signature-wrapper table { border-collapse: collapse !important; border-spacing: 0 !important; margin: 0 !important; padding: 0 !important; }
                   .crm-signature-wrapper img { display: block !important; }
                 </style>
                 ${signatureHtml}
              </div>`;
            signatureBlock = wrappedSig;
        }
      }
      ...
      const darkModeWrapper = `
        <div class="crm-email-content">
          <style>
            ...
            /* Email body text should be white, even when body has inline color:#222 */
            .crm-email-content [data-body="true"],
            .crm-email-content [data-body="true"] * {
              color: var(--text-primary, #ffffff) !important;
            }
            /* Reset paragraph margins (body only) */
            .crm-email-content [data-body="true"] p {
              margin-bottom: 16px;
            }
          </style>
          <div data-body="true">${decoded}</div>
          ${signatureBlock}
        </div>
      `;
