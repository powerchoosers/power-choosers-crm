...
class ActivityManager {
  constructor() {
    ...
    this._homeDeferredRefreshTimer = null;
    this._homeDeferredRefreshStartedAt = 0;
    this._homeDeferredRefreshLastTriggeredAt = 0;
    this._homeDeferredRefreshCooldownMs = 20000;
  ...
  _scheduleGlobalHomeDeferredRefresh(timedOutTypes = []) {
    if (this._homeDeferredRefreshTimer) return;
    const startAt = Date.now();
    this._homeDeferredRefreshStartedAt = startAt;

    const attempt = () => {
      const currentPage = window.crm?.currentPage || '';
      if (currentPage && currentPage !== 'dashboard' && currentPage !== 'home') {
        this._homeDeferredRefreshTimer = null;
        this._homeDeferredRefreshStartedAt = 0;
        return;
      }
      ...
      try {
        if (window.BackgroundEmailsLoader && typeof window.BackgroundEmailsLoader.getEmailsData === 'function') {
          const emails = window.BackgroundEmailsLoader.getEmailsData() || [];
          emailsReady = Array.isArray(emails) && emails.length > 0;
        }
      } catch (_) { }

      const emailsCached = this.processedEmailsCache && this.processedEmailsCache.size > 0;
      const elapsed = Date.now() - startAt;
      const lastTriggeredAt = Number(this._homeDeferredRefreshLastTriggeredAt) || 0;
      const cooldown = Number(this._homeDeferredRefreshCooldownMs) || 0;
      const now = Date.now();
      const maxWait = Math.max(20000, this.homeSourceTimeoutMs * 12);

      if ((emailsReady || emailsCached) && (!cooldown || (now - lastTriggeredAt) >= cooldown)) {
        this._homeDeferredRefreshTimer = null;
        this._homeDeferredRefreshStartedAt = 0;
        this._homeDeferredRefreshLastTriggeredAt = now;
        try {
          this.invalidateActivityCache('global', null);
        } catch (_) { }
        this.renderActivities('home-activity-timeline', 'global', null, true, { disableAnimations: true });
        return;
      }
      ...
      this._homeDeferredRefreshTimer = setTimeout(attempt, 1500);
    };

    this._homeDeferredRefreshTimer = setTimeout(attempt, 1500);
  }

  _waitForHomeEmailsReady(maxWaitMs = 2000) {
    const startAt = Date.now();
    const checkReady = () => {
      let emailsReady = false;
      try {
      ...
      const emailsCached = this.processedEmailsCache && this.processedEmailsCache.size > 0;
      return emailsReady || emailsCached;
    };

    if (checkReady()) return Promise.resolve(true);

    return new Promise(resolve => {
      const loop = () => {
        const elapsed = Date.now() - startAt;
        if (checkReady() || elapsed >= maxWaitMs) {
          resolve(checkReady());
          return;
        }
        setTimeout(loop, 150);
  ...
  loadPersistedCache(key) {
    // Cache versioning to force invalidation when logic changes
    const CACHE_VERSION = '1.3'; 
    try {
      // Check version only once per session load (conceptually)
      // But since we load multiple caches, we can check it every time or just rely on the first check clearing everything
      const storedVersion = sessionStorage.getItem('activityManager_version');
      if (storedVersion !== CACHE_VERSION) {
        console.log(`[ActivityManager] Cache version mismatch (stored: ${storedVersion}, current: ${CACHE_VERSION}). Clearing caches.`);
        sessionStorage.removeItem('activityManager_processedActivities');
        sessionStorage.removeItem('activityManager_processedEmails');
        sessionStorage.removeItem('activityManager_pageCache');
        sessionStorage.setItem('activityManager_version', CACHE_VERSION);
        return null;
      }
  ...
  setupCacheInvalidationListeners() {
    ...
    // Invalidate caches when emails are updated (sent/received/scheduled)
    document.addEventListener('pc:emails-updated', (e) => {
      // Invalidate global activities to pick up new emails
      this.invalidateActivityCache('global', null);
      this.processedEmailsCache.clear(); // Clear processed emails cache
      
      // Refresh visible containers (dashboard timeline)
      this.refreshVisibleActivityContainers('global', null, { 
        source: 'pc:emails-updated',
        reason: e.detail?.source || 'unknown'
      });
    });

    // Handle real-time tracking updates (opens/clicks)
    document.addEventListener('pc:email-tracking-updated', (e) => {
      const { contactId, accountId } = e.detail || {};
      
      // Invalidate relevant caches
      this.invalidateActivityCache('global', null);
      if (contactId) this.invalidateActivityCache('contact', contactId);
      if (accountId) this.invalidateActivityCache('account', accountId);
      this.processedEmailsCache.clear();

      // Refresh visible containers
      this.refreshVisibleActivityContainers('global', null, { source: 'pc:email-tracking-updated' });
      if (contactId) this.refreshVisibleActivityContainers('contact', contactId, { source: 'pc:email-tracking-updated' });
    });
      ...
      try {
        ...
        if (isGlobalHome) {
          const wrap = async (label, promise) => {
            ...
            let data = [];
            try {
              const result = await this._withTimeout(promise, this.homeSourceTimeoutMs, label);
              data = Array.isArray(result) ? result : [];
            } catch (_) {
              timedOut = true;
              data = [];
            }
            return { label, data, timedOut };
          };

          const results = await Promise.all([
            wrap('notes', this.getNoteActivities(entityType, entityId, perTypeLimit, forceRefresh)),
            wrap('sequences', this.getSequenceActivities(entityType, entityId, perTypeLimit, forceRefresh)),
            wrap('emails', this.getEmailActivities(entityType, entityId, perTypeLimit, forceRefresh)),
            wrap('tasks', this.getTaskActivities(entityType, entityId, perTypeLimit, forceRefresh))
          ]);

          const timedOutTypes = results.filter(r => r.timedOut).map(r => r.label);
          if (timedOutTypes.includes('emails')) {
            this._scheduleGlobalHomeDeferredRefresh(timedOutTypes);
          }

          notes = results[0].data;
          sequences = results[1].data;
          emails = results[2].data;
          tasks = results[3].data;
        } else {
          const results = await Promise.all([
            this.getNoteActivities(entityType, entityId, perTypeLimit, forceRefresh),
            this.getSequenceActivities(entityType, entityId, perTypeLimit, forceRefresh),
            this.getEmailActivities(entityType, entityId, perTypeLimit, forceRefresh),
            this.getTaskActivities(entityType, entityId, perTypeLimit, forceRefresh)
          ]);
          notes = results[0];
          sequences = results[1];
          emails = results[2];
  ...
  async getEmailActivities(entityType, entityId, limit, forceRefresh = false) {
    ...
    try {
      ...
      if (entityType === 'global') {
        ...
        if (emails.length === 0) {
          if (window.CacheManager) {
            emails = await window.CacheManager.get('emails', forceRefresh);
          } else {
            emails = await this.fetchEmails(limit);
          }
        }
      ...
      const processEmailBatch = async (emails, startIndex = 0, batchSize = 50) => {
        ...
        batch.forEach(email => {
          ...
          // Strip tracking pixel from content to prevent false opens when viewing in CRM
          // This removes the 1x1 pixel image that triggers the "read" status
          const stripTracking = (content) => {
            if (!content) return content;
            // Matches <img src=".../api/email/track/..." ...>
            return String(content).replace(/<img[^>]*src=["'][^"']*\/api\/email\/track\/[^"']*["'][^>]*>/gi, '');
          };

          const cleanEmail = { ...email };
          if (cleanEmail.html) cleanEmail.html = stripTracking(cleanEmail.html);
          if (cleanEmail.body) cleanEmail.body = stripTracking(cleanEmail.body);
          if (cleanEmail.content) cleanEmail.content = stripTracking(cleanEmail.content);
  ...
  async fetchEmails(limit = this.fetchLimitPerType) {
    try {
      ...
      if (window.BackgroundEmailsLoader && typeof window.BackgroundEmailsLoader.getEmailsData === 'function') {
        ...
        // Check by contactId first (fastest)
        // CRITICAL FIX: If contacts haven't loaded yet (size === 0), trust the contactId to avoid hiding emails on dashboard load
        if (email.contactId) {
            if (contactIdsSet.size === 0 || contactIdsSet.has(email.contactId)) {
                return true;
            }
        }
  ...
  async renderActivities(containerId, entityType = null, entityId = null, forceRefresh = false, opts = {}) {
    ...
    const renderPromise = (async () => {
      ...
      try {
        const isHomeTimeline = containerId === 'home-activity-timeline' && (entityType === 'global' || !entityType);
        if (isHomeTimeline && !forceRefresh && !hasExistingContent) {
          const waitMs = Math.min(Number(this.homeSourceTimeoutMs) || 0, 2000);
          if (waitMs > 0) {
            await this._waitForHomeEmailsReady(waitMs);
          }
        }
  ...
  /**
   * Strip tracking pixel from HTML content
   */
  stripTrackingPixel(content) {
    if (!content) return content;
    // Matches <img src=".../api/email/track/..." ...>
    // Robust regex to handle various attribute orderings and quoting styles
    // Matches <img ... src=".../api/email/track/..." ... >
    return String(content).replace(/<img[^>]*src=["'][^"']*\/api\/email\/track\/[^"']*["'][^>]*>/gi, '');
  }
  ...
  stripHtml(html) {
    ...
    try {
      // Use DOMParser to safely parse HTML without executing scripts or loading images
      ...
      // Fallback: Remove style, script, and img tags regex
      let cleaned = html
