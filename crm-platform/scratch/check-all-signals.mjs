...
function getTopicKeywords(headline) {
...
async function verifySignal(row) {
  ...
  // Skip deep verification for SEC filings or EDGAR searches since they are generic databases
  ...
  const keywords = getTopicKeywords(headline);
  ...
  try {
    ...
    try {
      ...
      return { ok: false, reason: `Network error on both: ${directErr.message}`, code: 0 };
    }
  }
  ...
  if (!isHomepage && keywords.length > 0) {
    ...
    // If we match 0 keywords, it's very likely a different/cookie/redirect page
...
async function run() {
  console.log("Fetching recent market intelligence rows...");
  ...
  for (const row of rows) {
    console.log(`Checking [${row.entity_name}] - ${row.headline}`);
    const res = await verifySignal(row);
    if (!res.ok) {
      console.log(`❌ FAILED: ${res.reason} (URL: ${row.source_url})`);
      results.push({ row, reason: res.reason, code: res.code });
    } else {
      console.log(`✅ PASSED (Status ${res.code})`);
    }
  }

  console.log(`\nScan complete. Found ${results.length} failures.`);
  fs.writeFileSync('scratch/scan-failures.json', JSON.stringify(results, null, 2));
  console.log("Failures written to scratch/scan-failures.json");
}
