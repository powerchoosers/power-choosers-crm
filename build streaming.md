...
- **Auth:** Zoho OAuth (login), Supabase users table (profile/sessions)
- **Backend:** Next.js API Routes at `crm-platform/src/pages/api/` (Twilio, ERCOT/EIA, Gemini AI, Zoho Mail)
...
**Supabase (PostgreSQL)** primary, Firestore legacy. Tables: `users`, `accounts`, `contacts`, `deals`, `calls`, `emails`, `tasks`, `market_telemetry`, `apollo_news_articles`, `signature_requests`, `signature_telemetry`.
...
**War Room (Dashboard):** `/network` - The main command center featuring the Priority Stack (expiring accounts), Signal Matrix, Open Positions, and live ERCOT GridStrip.
...
**Voice & Messaging Platform:** Twilio integration, click-to-call, live transcription (AssemblyAI), WhatsApp (OpenClaw), voicemail inbox.

**Email System:** Zoho Mail API, threaded conversations, draft templates, Apollo.io enrichment, AI Sequence generation.

**Contract Execution:** Secure Portal (`/secure-portal/sign/[token]`), real-time `signature_telemetry` tracking, IP/UserAgent forensics, dynamic PDF proposal generation.
...
- `/api/email/*` - Zoho Mail sync, composition, sequence routing
- `/api/auth/zoho/*` - Zoho OAuth authentication
- `/api/signatures/*` - Contract generation, signing, and telemetry tracking
