      ...
      switch (action) {
        ...
        case 'templates': {
          try {
            ...
            const initialHtml = bodyInput ? bodyInput.innerHTML : '';

            if (window.PCEmailBuilder && typeof window.PCEmailBuilder.open === 'function' && bodyInput && subjectInput) {
              window.PCEmailBuilder.open({
                source: 'email-compose-global',
                subject: subject,
                initialHtml: initialHtml,
                onSave: function (result) {
                  const s = (result && result.subject) || subject;
                  const html = (result && result.html) || initialHtml;
                  subjectInput.value = s;
                  bodyInput.setAttribute('data-html-email', 'true');
                  try {
                    renderHtmlEmailInIframe(bodyInput, html);
                  } catch (e) {
              ...
              window.crm?.showToast('HTML email builder not available yet');
            }
          } catch (e) {
            console.error('[EmailCompose] Templates handler failed', e);
            window.crm?.showToast('Unable to open templates');
          }
          break;
        }
        default: {
...
<body>
  ...
  function setupMainToolbarHandler() {
    ...
    document.addEventListener('click', (e) => {
      ...
      // Skip if button is part of sequence builder or other modules
      if (btn.closest('.step-card') || btn.closest('.sequence-steps')) return;
