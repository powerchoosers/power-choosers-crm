...
(function () {
  ...
  function renderAccountDetail() {
          ...
          <div class="info-grid" id="account-service-addresses-grid">
            ...
            <div class="info-row">
              <div class="info-label" style="grid-column: span 2;">
                <button class="btn-text-bordered" id="add-service-address" style="display: flex; align-items: center; gap: 6px; padding: 6px 12px; margin-top: 8px;">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    try {
      if (state._arcRefreshInterval) {
        clearInterval(state._arcRefreshInterval);
        state._arcRefreshInterval = null;
      }
    } catch (_) { }
    if (typeof state._arcLastRefreshTime !== 'number') state._arcLastRefreshTime = 0;
    const startPeriodicRefresh = () => {
      try {
        if (state._arcRefreshInterval) clearInterval(state._arcRefreshInterval);
      } catch (_) { }
      state._arcRefreshInterval = setInterval(() => {
        ...
        const now = Date.now();
        const timeSinceLastRefresh = now - (Number(state._arcLastRefreshTime) || 0);

        if (!state._arcReloadInFlight && !state._isScrolling && !hasOpenInsights && timeSinceLastRefresh >= 60000) {
          state._arcLastRefreshTime = now;
    ...
    if (!document._arcBeforeUnloadBound) {
      ...
      document._arcBeforeUnloadBound = '1';
    }
  ...
  function onAnyAccountCallActivity() {
    ...
    // If list is empty and user isn't viewing any details panels, show loading overlay
    try {
      const list = document.getElementById('account-recent-calls-list');
      const hasOpen = (state._arcOpenIds && state._arcOpenIds.size > 0);
      const hasRows = !!(list && list.querySelector && list.querySelector('.rc-item'));
      if (list && !hasOpen && !hasRows) arcSetLoading(list);
    } catch (_) { }
  ...
  async function loadRecentCallsForAccount() {
    ...
    try {
      const hasRows = !!(list.querySelector && list.querySelector('.rc-item'));
      if (!hasRows) arcSetLoading(list);
    } catch (_) { }
  ...
  function arcRenderPage() {
    ...
    try {
      ...
      const sig = `${String(state.currentAccount && state.currentAccount.id || '')}::p${String(state._arcPage || 1)}::${key}`;
  ...
  function insightsInlineHtml(r) {
    ...
    const audio = proxied ? `<audio controls preload="metadata" style="width:100%; margin-top:8px;"><source src="${proxied}" type="audio/mpeg">Your browser does not support audio playback.</audio>` : '<div style="color:var(--text-muted); font-size:12px;">No recording available</div>';
