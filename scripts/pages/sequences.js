...
(function () {
  ...
  async function waitForBackgroundSequences(timeoutMs) {
    return new Promise((resolve) => {
      let done = false;
      const onLoaded = () => {
        if (done) return;
        done = true;
        document.removeEventListener('pc:sequences-loaded', onLoaded);
        resolve(true);
      };
      document.addEventListener('pc:sequences-loaded', onLoaded);
      setTimeout(() => {
        if (done) return;
        done = true;
        document.removeEventListener('pc:sequences-loaded', onLoaded);
  ...
  async function ensureSequencesPreloaded(timeoutMs) {
    const coordinator = window.BackgroundLoaderCoordinator;
    if (!coordinator) return false;
    if (typeof coordinator.isLoaded === 'function' && coordinator.isLoaded('sequences')) return true;
    if (typeof coordinator.loadCollection !== 'function') return false;
    const loadPromise = coordinator.loadCollection('sequences')
      .then(() => true)
      .catch(() => false);
  ...
  async function directLoadSequences() {
    if (window.DataManager && typeof window.DataManager.queryWithOwnership === 'function') {
      const owned = await window.DataManager.queryWithOwnership('sequences');
    ...
    const liveDb = window.firebaseDB;
    const liveCol = liveDb ? liveDb.collection('sequences') : null;
    if (!liveCol) {
      return null;
    }
    const snap = await liveCol.get();
    const items = [];
    snap.forEach(doc => {
      const data = doc.data() || {};
  ...
  async function loadFromFirestore() {
    if (window.BackgroundSequencesLoader) {
      let sequencesData = window.BackgroundSequencesLoader.getSequencesData() || [];
      let hasMore = typeof window.BackgroundSequencesLoader.hasMore === 'function' 
        ? window.BackgroundSequencesLoader.hasMore() 
        : false;

      if (sequencesData.length === 0) {
        const waited = await waitForBackgroundSequences(200);
        if (waited) {
          sequencesData = window.BackgroundSequencesLoader.getSequencesData() || [];
          hasMore = typeof window.BackgroundSequencesLoader.hasMore === 'function' 
            ? window.BackgroundSequencesLoader.hasMore() 
            : false;
        }
      }
    ...
    const items = await directLoadSequences();
    if (!items) return;
    state.data = items;
  ...
  function attachEvents() {
    ...
    if (els.modalCreate) {
      els.modalCreate.addEventListener('click', () => {
        ...
        const finalUserEmail = getUserEmail();
        ...
        const baseSeq = {
          ...
          ownerId: finalUserEmail,
          assignedTo: finalUserEmail,
          isActive: false,
  ...
  function skeletonHtml(count) {
    const rows = Array(Math.max(1, count)).fill(0).map(() => {
      const cells = [
        `<td class="col-select"><div class="skeleton-text skeleton-shimmer"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer medium"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer short"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer short"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer short"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer short"></div></td>`,
        `<td><div class="skeleton-text skeleton-shimmer short"></div></td>`,
  ...
  function renderSkeleton() {
    if (!els.tbody) return;
    const count = Math.max(6, state.pageSize);
    els.tbody.setAttribute('data-skeleton', '1');
    els.tbody.innerHTML = skeletonHtml(count);
    if (els.pagination) els.pagination.innerHTML = '';
    if (els.paginationSummary) els.paginationSummary.textContent = 'Loading sequences…';
  }

  function render() {
    if (!els.tbody) return;
    if (state.loading && !state.loaded && (!state.data || state.data.length === 0)) {
      renderSkeleton();
      return;
    }
    els.tbody.removeAttribute('data-skeleton');
  ...
  async function init() {
    ...
    if (!document._sequencesVisibilityBound && els.page) {
      document._sequencesVisibilityBound = true;
      const observer = new MutationObserver(() => {
        if (!els.page) return;
        const visible = !els.page.hidden && els.page.classList.contains('active');
        if (visible && state.needsRender) {
          applyFilters();
          state.needsRender = false;
        }
      });
      observer.observe(els.page, { attributes: true, attributeFilter: ['class', 'hidden'] });
    }
    ...
    if (!document._sequencesLoadedBound) {
      onBackgroundLoaded = async () => {
        try {
          ...
          // Only perform expensive render if page is visible or we have no data yet
          const page = document.getElementById('sequences-page');
          const shouldRender = (page && page.offsetParent !== null) || (!state.data || state.data.length === 0);
      ...
      // Listen for sequence member removal to update counts
      document.addEventListener('pc:sequence-member-removed', async (e) => {
        console.log('[Sequences] Sequence member removed event received:', e.detail);
        // Force immediate recalculation to ensure UI is in sync
        await recalculateSequenceCounts();
      });

      // Listen for sequence member addition to update counts
      document.addEventListener('pc:sequence-member-added', async (e) => {
        console.log('[Sequences] Sequence member added event received:', e.detail);
        // Force immediate recalculation to ensure UI is in sync
        await recalculateSequenceCounts();
      });
    ...
    try {
      ...
      await ensureSequencesPreloaded(2000);
  ...
  async function recalculateSequenceCounts() {
    ...
    try {
      ...
      if (Array.isArray(state.filtered) && state.filtered.length) {
        const byId = new Map((state.data || []).map(s => [String(s.id), s]));
        state.filtered = state.filtered.map(s => byId.get(String(s.id)) || s);
      }

      const page = document.getElementById('sequences-page');
      const shouldRender = page && page.offsetParent !== null;
      state.needsRender = true;

      if (fixedCount > 0) {
        if (shouldRender) render();
        if (shouldRender) state.needsRender = false;
        if (window.crm?.showToast) {
