...
class ToastManager {
        ...
        if (customIcon) {
            ...
            if (trimmedIcon && (trimmedIcon.startsWith('http') || trimmedIcon.startsWith('data:image'))) {
                ...
                img.onerror = () => {
                    ...
                    if (type === 'call') {
                        // Use initials if available, otherwise '?'
                        const initials = this._lastInitials || '?';
    ...
    showCallNotification(callData) {
        ...
        const initials = callerName ? callerName.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2) : '';
        this._lastInitials = initials; // Store for fallback
        ...
        if (isCompanyPhone && window.__pcFaviconHelper) {
            ...
            // Fallback to image if no name available (e.g. unknown contact with domain hint)
            icon = callerIdImage;
        } else if (carrierName) {
    ...
    showEmailGeneratedNotification(emailData) {
        const { contactName, subject, sequenceName, count, message: customMessage } = emailData;

        // AI/Sparkles icon - same style as "Write with AI" in email compose
        // Using layers/sparkles icon to represent AI generation
        const aiIcon = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z"></path><path d="M2 17l10 5 10-5"></path><path d="M2 12l10 5 10-5"></path></svg>`;

        let title = 'Email Generated';
        let message = '';
        let details = '';
        ...
        if (customMessage) {
            ...
            message = `${count} emails ready for review`;
        } else {
            // Single email
            message = subject || 'Email content ready for review';
        }

        // Build details line
        if (contactName) {
            details = `For ${contactName}`;
            if (sequenceName) {
    ...
    answerCall(callData) {
        ...
        try {
            if (window.Widgets) {
                // Pass full CRM context to the phone widget so it populates correctly
                if (typeof window.Widgets.setCallContext === 'function') {
                    window.Widgets.setCallContext({
                        ...
                        domain: callData.domain,
                        logoUrl: callData.callerIdImage, // This now includes the fallback
                        isActive: true
                    });
                }

                if (typeof window.Widgets.openPhone === 'function') {
