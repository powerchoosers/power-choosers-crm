...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  for (const contact of contacts || []) {
    ...
    try {
      // 1. Attempt to generate an invite link (for new users)
      ...
      if (!inviteError && inviteData?.properties?.action_link) {
        ...
        if (magicError || !magicData?.properties?.action_link) {
          results.push({ contactId: contact.id, email: contact.email, status: 'error', message: inviteError?.message || magicError?.message || 'Could not generate link' })
          continue
        }
      ...
      if (!actionLink) {
        results.push({ contactId: contact.id, email: contact.email, status: 'error', message: 'Could not generate access link' })
        continue
      }
