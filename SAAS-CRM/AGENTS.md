...
Nodal Point is the first "Apollo for Energy"—a high-fidelity prospecting and forensic intelligence platform built specifically for commercial energy consultants. While it started as an internal CRM, it is now a multi-tenant SaaS platform that empowers brokers to spot hidden cost leakage, understand account risk via forensic data, and automate hyper-personalized outreach.
...
## Multi-Tenancy & SaaS Rules

Nodal Point is a multi-tenant platform. All data must be scoped to an organization.

- **Organization Scoping**: Every core table (`accounts`, `contacts`, `calls`, `emails`, `sequences`) must have an `orgId` (or `organization_id`).
- **RLS**: Supabase Row Level Security (RLS) is the primary line of defense. Ensure all queries include the current user's `orgId`.
- **User Roles**: Support for `admin`, `member`, and `readonly` roles per organization.
- **Credit System**: Implementation of a "Credit" system for data exports and sequence sends (similar to Apollo).

## Data & Intelligence Rules

- **Forensic First**: We don't just provide contact info; we provide energy-specific "Forensic Sound Profiles" (e.g., thermal liability, load factor anomalies).
- **Enrichment**: Prefer energy-specific data sources (EIA, ERCOT, nodal pricing) over generic B2B data when possible.
- **Normalization**: Resolve fields in this order:
  1. top-level camelCase
  2. underscored legacy fields
  3. nested values inside `metadata`
- **Ownership**: `contacts.accountId` links to `accounts.id`. `ownerId` should align with Supabase auth UUID for RLS.
...
- **Think Platform, Not App**: We are building a platform for *other* consultants. Avoid hardcoding logic that only works for one specific business.
- **Forensic Authority**: Preserve the forensic, command-center aesthetic. It should look like a professional energy trading/intelligence tool, not a generic CRM.
- **Consistency**: Match local patterns. If adding a new feature, look at how `/network` or `/protocols` is built first.
- **Performance**: High-density data must be performant. Use TanStack Query effectively.
- **Safety**: Never bypass RLS. Always check the `orgId` context.
