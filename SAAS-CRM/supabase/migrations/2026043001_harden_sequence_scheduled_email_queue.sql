-- Keep protocol scheduled email previews safe:
-- 1) sequence/cold email previews always show the burner sender domain
-- 2) raw protocol templates with {{variables}} are not exposed as sendable copy
-- 3) metadata marks sequence email rows consistently for throttling/reporting
...
DECLARE
  ...
  v_owner_id TEXT;
  v_owner_email TEXT;
  v_sequence_sender_email TEXT;
  v_from_email TEXT;
  v_subject TEXT;
  v_body TEXT;
  v_body_text TEXT;
  v_needs_generation BOOLEAN := FALSE;
...
BEGIN
  ...
  SELECT
    ...
    u.email,
    COALESCE(
      NULLIF(s.bgvector->'settings'->>'senderEmail', ''),
      NULLIF(s.metadata->>'sender_email', '')
    )
  INTO
    v_contact_id,
    v_account_id,
  ...
  v_from_email := COALESCE(
    NULLIF(NEW.metadata->>'senderEmail', ''),
    NULLIF(NEW.metadata->>'from', ''),
    NULLIF(v_sequence_sender_email, ''),
    NULLIF(v_owner_email, ''),
    CASE WHEN POSITION('@' IN COALESCE(v_owner_id, '')) > 0 THEN v_owner_id ELSE NULL END
  );

  IF COALESCE(v_from_email, '') <> '' THEN
    v_from_email := regexp_replace(v_from_email, '@nodalpoint\.io$', '@getnodalpoint.com', 'i');
  END IF;
  ...
  IF v_subject ~ '\{\{\s*[^}]+\s*\}\}' OR v_body ~ '\{\{\s*[^}]+\s*\}\}' THEN
    v_needs_generation := TRUE;
    v_body := '';
    v_body_text := '';
    v_subject := COALESCE(NULLIF(NEW.metadata->>'label', ''), 'Needs AI generation');
  ELSE
  ...
  IF FOUND
    ...
    AND COALESCE(v_existing_email.metadata->>'sequenceExecutionId', '') = NEW.id::text
    AND COALESCE(v_existing_email.metadata->>'sequenceId', '') = NEW.sequence_id::text
    AND COALESCE(v_existing_email.metadata->>'memberId', '') = NEW.member_id::text
    ...
    AND COALESCE(v_existing_email.metadata->>'needsGeneration', 'false')::boolean IS NOT DISTINCT FROM v_needs_generation
  THEN
    RETURN NEW;
  END IF;
  ...
  INSERT INTO public.emails (
    ...
    jsonb_build_object(
      ...
      'needsGeneration', v_needs_generation,
