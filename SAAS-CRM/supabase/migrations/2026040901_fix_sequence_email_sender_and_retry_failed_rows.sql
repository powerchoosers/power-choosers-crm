-- Ensure scheduled sequence placeholders use the protocol sender email, not the owner mailbox.
-- Also let the existing requeue job recover transient failed email steps with a bounded retry count.
