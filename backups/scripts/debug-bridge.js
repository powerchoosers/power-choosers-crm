(function() {
    ...
    const debugEnabledAtLoad = (function () {
        try {
            return (
    ...
    try {
        ...
        window.PCDebug.enable = function () {
            ...
            try { localStorage.setItem('pc-debug-optin', 'true'); } catch (_) { }
        ...
        window.PCDebug.disable = function () {
            ...
            try { localStorage.removeItem('pc-debug-optin'); } catch (_) { }
    ...
    try {
        fetch('/api/debug/log', {
            ...
            body: JSON.stringify({
                type: 'log',
                message: '[Debug Bridge] Heartbeat - Bridge initialized. UA: ' + navigator.userAgent,
                url: window.location.href,
                timestamp: new Date().toISOString()
            }),
            keepalive: true
