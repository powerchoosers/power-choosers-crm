  ...
  async function addCurrentContactToSequence(sequenceId, sequenceName) {
    try {
      ...
      if (db && typeof db.collection === 'function') {
        ...
        // Increment sequence stats.active count and recordCount
          ...
          if (window.BackgroundSequencesLoader && typeof window.BackgroundSequencesLoader.getSequencesData === 'function') {
            ...
            if (seq) {
              ...
              document.dispatchEvent(new CustomEvent('pc:sequences-loaded', {
                detail: { count: sequences.length, fromCache: true }
              }));
              
              // Dispatch specific event for sequence member added
              document.dispatchEvent(new CustomEvent('pc:sequence-member-added', {
                detail: { sequenceId, contactId, source: 'contact-detail' }
              }));
            }
          }
        } catch (e) {
    ...
    try {
      ...
      if (db && typeof db.collection === 'function' && memberDocId) {
        ...
        if (contactId && sequenceId) {
          try {
            ...
            if (!activationsSnapshot.empty) {
              ...
              console.log('[ContactDetail] Deleted old sequenceActivations for contact:', contactId);
            }
            ...
            if (!emailsSnapshot.empty) {
              ...
              console.log('[ContactDetail] Deleted old scheduledEmails for contact:', contactId);
            }
            ...
            console.warn(`[ContactDetail] Failed to cleanup sequence data for contact ${contactId}:`, cleanupError);
          }
        }
        ...
        if (sequenceId && window.firebase?.firestore?.FieldValue) {
          ...
          try {
            const seqSnap = await sequenceRef.get();
            if (seqSnap.exists) {
              const seqData = seqSnap.data() || {};
              if (Array.isArray(seqData.contacts)) {
                updatedContacts = seqData.contacts.filter(c => String(c?.id || c) !== String(contactId));
              }
            }
          } catch (e) {
            console.warn('Failed to load sequence contacts for cleanup:', e);
          }
          ...
          if (updatedContacts) {
            updatePayload.contacts = updatedContacts;
          }
          await sequenceRef.update(updatePayload);

          try {
            if (window._sequenceMembersCache) {
              window._sequenceMembersCache.delete(`sequence-members-${sequenceId}`);
            }
          } catch (_) { }
            ...
            if (window.BackgroundSequencesLoader && typeof window.BackgroundSequencesLoader.getSequencesData === 'function') {
              ...
              if (seq) {
                ...
                if (window.CacheManager && typeof window.CacheManager.updateRecord === 'function') {
                  ...
                  if (updatedContacts) {
                    updateRecord.contacts = updatedContacts;
                  }
                  window.CacheManager.updateRecord('sequences', sequenceId, updateRecord);
                }
