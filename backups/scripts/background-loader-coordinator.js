...
class BackgroundLoaderCoordinator {
  ...
  async coordinateLoading() {
    ...
    try {
      ...
      // PRIORITY 1: Dashboard-critical collections
      // These must load first because they power the dashboard widgets
      const critical = ['tasks', 'emails'];
      for (const collection of critical) {
        const startTime = Date.now();
        await this.loadCollection(collection);
        this.loadStats.set(collection, Date.now() - startTime);
      }

      // PRIORITY 2: Core data collections (independent)
      // These can load in parallel after critical items are ready
      const independent = ['contacts', 'accounts', 'calls', 'sequences', 'lists'];
